      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card card-primary">
            ...
            <form method="POST" action="{{ route('componet_update', ['id' => $component->id]) }}">
               @csrf
               @method('PUT')
              <div class="card-body">
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                  ...
                  <select class="form-control select2 select2-hidden-accessible" name="project" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
                    ...
                    @foreach ($projects as $key => $project)
                      <option data-select2-id="39" value="{{$project->id}}" @if($projects->project_id == $project->id) selected @endif>{{$project->name}}</option>
                    @endforeach
                  </select>
