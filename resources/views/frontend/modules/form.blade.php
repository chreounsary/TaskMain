      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card card-primary">
            ...
            <form method="POST" action="{{ route('module_store') }}">
               @csrf
              <div class="card-body">
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                  ...
                  @endif
                  <input type="hidden" name="project_id" value="{{ $tasks->project_id }}">
                  ...
                  <select class="form-control select2 select2-hidden-accessible" name="project" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
                    <option selected="selected" data-select2-id="3">--Select--</option>
                     @foreach ($tasks as $key => $task)
                        <option data-select2-id="39" value="{{$task->id}}" @if($tasks->task_id == $task->id) selected @endif>{{$task->name}}</option>
                      @endforeach
                  </select>
                </div>
